...
if (-not (Test-Path (Join-Path $Repo '.git'))) { Write-Error "claude-memory introuvable dans $Repo"; exit 0 }
Set-Location $Repo

# Sécurité : refuser de pousser si le dépôt n'est pas privé.
if (Get-Command gh) {
  $vis = gh repo view --json visibility -q .visibility
  if ($vis -and $vis -ne 'PRIVATE') { Write-Error "claude-memory n'est pas privé ($vis)"; exit 0 }
}
...
if ($hits) {
  Write-Error "Secret potentiel détecté, rien n'est poussé :"; $hits | ForEach-Object { Write-Error $_.Path }
